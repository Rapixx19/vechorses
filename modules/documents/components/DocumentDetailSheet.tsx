 ...
 * ZONE: Green
 * PURPOSE: Detail sheet for viewing document info and AI analysis
...
const statusConfig: Record<DocumentStatus, { icon: typeof CheckCircle; color: string; label: string }> = {
  valid: { icon: CheckCircle, color: "text-green-400 bg-green-400/10", label: "Valid" },
  expiring: { icon: AlertTriangle, color: "text-yellow-400 bg-yellow-400/10", label: "Expiring Soon" },
  expired: { icon: AlertTriangle, color: "text-red-400 bg-red-400/10", label: "Expired" },
}
...
export function DocumentDetailSheet({
  ...
  const daysUntilExpiry = document.expiryDate
    ? Math.ceil((new Date(document.expiryDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
    : null

  const handleAnalyse = async () => {
    if (!document.fileUrl) {
      setAiError("No file URL to analyse")
      return
    }
      ...
      >
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="badge badge-blue text-xs">{categoryLabels[document.category] || document.category}</span>
                <span className={`badge ${statusConfig[document.status].color} text-xs flex items-center gap-1`}>
        ...
        <div className="p-4 space-y-6">
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-medium text-[var(--text-muted)] uppercase tracking-wider">Details</h3>
            ...
            {document.expiryDate && (
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className={`${document.status === "expired" ? "text-red-400" : document.status === "expiring" ? "text-yellow-400" : "text-[var(--text-primary)]"}`}>
            ...
            {document.issuedBy && (
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="text-[var(--text-primary)]">{document.issuedBy}</span>
              </div>
            )}
            ...
            {document.referenceNumber && (
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="text-[var(--text-primary)] font-mono">{document.referenceNumber}</span>
              </div>
            )}
          ...
          <div className="card p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-medium text-[var(--text-primary)]">AI Summary</h3>
            </div>

            {document.aiSummary ? (
              <p className="text-sm text-[var(--text-secondary)]">{document.aiSummary}</p>
            ) : (
              <p className="text-sm text-[var(--text-muted)]">
                No AI summary yet. Click the button below to analyse this document.
              </p>
            )}

            {aiError && (
              <p className="text-sm text-[var(--red)]">{aiError}</p>
            )}

            <button
              onClick={handleAnalyse}
              disabled={isAnalysing || !document.fileUrl}
            ...
            {!document.fileUrl && (
              <p className="text-xs text-[var(--text-muted)]">
                Add a file URL to enable AI analysis.
              </p>
            )}
          </div>
        </div>
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 z-60 flex items-center justify-center p-4" onClick={() => setShowDeleteConfirm(false)}>
            <div className="absolute inset-0 bg-black/50" />
            <div className="relative bg-[var(--bg-surface)] rounded-lg p-6 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
              <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-2">Delete Document?</h3>
              <p className="text-sm text-[var(--text-muted)] mb-4">
                Are you sure you want to delete &quot;{document.title || document.name}&quot;? This action cannot be undone.
              </p>
