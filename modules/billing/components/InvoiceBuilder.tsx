...
import { X, Plus, Eye, Download, Send, Save } from "lucide-react"
...
export function InvoiceBuilder({ clientId, client, lineItems, settings, onClose, onSave }: InvoiceBuilderProps) {
  ...
  const invoice: Invoice = useMemo(() => ({
    id: `inv-${Date.now()}`, invoiceNumber, clientId, lineItems: includedItems, subtotal, tax: tax || undefined,
    taxRate: taxRate || undefined, total, status: "draft", issuedDate: issueDate, dueDate, notes: notes || undefined, createdAt: new Date().toISOString(),
  }), [invoiceNumber, clientId, includedItems, subtotal, tax, taxRate, total, issueDate, dueDate, notes])
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80" onClick={() => setShowPreview(false)}>
        <div className="relative max-h-[90vh] overflow-auto bg-white rounded-lg" onClick={(e) => e.stopPropagation()}>
          ...
          <InvoicePreview invoice={invoice} client={client} settings={settings} />
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      <div className="relative flex w-full max-w-6xl mx-auto my-8 bg-[#0F1117] rounded-lg overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="w-1/2 p-4 overflow-y-auto bg-gray-100"><div className="transform scale-[0.6] origin-top"><InvoicePreview invoice={invoice} client={client} settings={settings} /></div></div>
      </div>
    </div>
  )
}
